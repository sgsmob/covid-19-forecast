...
ml.extract_slope_and_lag_columns_fn <- function(base_covariates, location_covariates) {
...
ml.linear <- function(train_test,
  ...
  # create new_train_X, new_test_X with slope variables
  extract_slope_and_lag_columns <-
    ml.extract_slope_and_lag_columns_fn(modeling_options$base_covariates,
                                        modeling_options$location_covariates)
  new_train_X <- extract_slope_and_lag_columns(train_X)
  new_test_X <- extract_slope_and_lag_columns(test_X)
  ...
  if (modeling_options$calibrate) {
    ...
    fit_train_y <- train_y[fit_inds,]
    fit_train_X <- extract_slope_and_lag_columns(train_X[fit_inds,])
    validate_train_y <- train_y[-fit_inds,]
    validate_train_X <- extract_slope_and_lag_columns(train_X[-fit_inds,])

    fit_quantiles <- quantgen::quantile_lasso(fit_train_X,
                                              fit_train_y,
                                              tau = tau,
                                              standardize = FALSE,
                                              lambda = 0)
    preds <- predict(fit_quantiles, validate_train_X, sort = TRUE)
